...
def velocity_planner(bot_pos):
    global v_tan_prev, v_norm_prev, kub
    #PD controller
    # angular velocity control
    if ROTATE:
        kub_pos = kub.state.awayPos[2]
    ...
    ball_vel = Vector2D(kub.state.ballVel.x, kub.state.ballVel.y)
    ball_speed = ball_vel.abs()
    max_bot_speed = config.MAX_BOT_SPEED - ball_speed
    if math.sqrt(vx**2+vy**2)>max_bot_speed:
        alpha = math.atan2(vy, vx)
        vx = max_bot_speed*math.cos(alpha)
        vy = max_bot_speed*math.sin(alpha)
    #print("vx,vy", vx, vy)
    vx = vx + ball_vel.x
    vy = vy + ball_vel.y
...
def execute(startTime,DIST_THRESH,ROTATION_FACTOR,avoid_ball=False):
    ...
    while not (FLAG_move and FLAG_turn):
        ...
        if not(prev_state == kub.state):
            ...
            if abs(functions.normalize_angle(kub.state.homePos[kub.kubs_id].theta-rotate))<0.25*ROTATION_FACTOR:
                kub.turn(0)
